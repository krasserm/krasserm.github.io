...
- Use the CNN to extract 128-dimensional representations, or *embeddings*, of faces from the aligned input images. In the Euclidean embedding space, distance directly corresponds to face similarity. 
- Compare input embedding vectors to labeled embedding vectors in a database. Here, a support vector machine (SVM) and a KNN classifier, trained on labelled embedding vectors, play the role of a database. Face recognition in this context means predicting the label of new inputs.
